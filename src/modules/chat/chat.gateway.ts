...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async onFindAllMessages(
    ...
    try {
      ...
      this.logger.error(
        `Failed to fetch messages for Room ID ${room_id} by User ID ${userId}: ${error.message}`,
        error.stack,
      );
  ...
  async onDeleteMessage(
    ...
    try {
      ...
      this.logger.log(
        `Messages deleted successfully in Room ID ${room_id} by User ID ${userId}. Notifications sent to all participants.`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to delete messages in Room ID ${room_id} by User ID ${userId}: ${error.message}`,
        error.stack,
      );
